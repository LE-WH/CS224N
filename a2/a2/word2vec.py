...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    score = softmax(np.dot(outsideVectors, centerWordVec))
    loss = -np.log(score[outsideWordIdx])
    onehot = np.zeros(outsideVectors.shape[0])
    onehot[outsideWordIdx] = 1
    gradCenterVec = np.dot(np.transpose(outsideVectors), score - onehot)
    gradOutsideVecs = np.dot(centerWordVec.reshape(-1, 1), score.reshape(1, -1))
...
def negSamplingLossAndGradient(
    ...
    U[1:, :] = - U[1:, :]
    tmp = sigmoid(np.dot(U, centerWordVec)) - 1
    loss = -sum(np.log(tmp + 1))
    U = np.transpose(U)
    gradCenterVec = np.dot(U, tmp)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx, :] = tmp[0] * np.transpose(centerWordVec)
    for ind, i in enumerate(indices[1:]):
        gradOutsideVecs[i, :] = gradOutsideVecs[i, :] - tmp[ind + 1] * np.transpose(centerWordVec)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentIndex = word2Ind[currentCenterWord]
    centerWordVec = np.transpose(centerWordVectors)[:, currentIndex]
    for item in outsideWords:
        outsideWordIdx = word2Ind[item]
        loss1, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient \
            (centerWordVec=centerWordVec, outsideWordIdx=outsideWordIdx, outsideVectors=outsideVectors, dataset=dataset)
        loss += loss1
        gradCenterVecs[currentIndex, :] += gradCenterVec
